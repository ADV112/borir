...
$(document).ready(function () {
		...
		'columns': [{
			...
			},
			{
				...
				render: function (data, type, full, meta) {
					...
					return '<img src="' + mainUrl + '/' + data.image + '" class="img-fluid">';
				}
			},
			{
				data: 'description'
	...
	$('#banner_filter > label > input').removeClass('form-control-sm').addClass("w-100").attr('placeholder', 'Search').parent().addClass('w-100');
	...
	$('#addBanner').submit(function (e) {
		e.preventDefault();
		if (check_form('#addBanner')) {
			$.ajax({
				url: globalurl + '/add-banner',
				type: 'POST',
				data: {
	...
	banner.on('click', '.delete', function () {
		var id = banner.row($(this).parents('tr')).data();
		$('#deleteHidden').val(id.id_content);
		$('#deleteModal').modal('toggle');
	});

	$('#delete-submit').click(function () {
		$.ajax({
			url: globalurl + '/delete-banner',
			type: 'POST',
			data: {
				id: $('#deleteHidden').val(),
				_token: $('input[name=_token]').val()
