...
class MY_Model extends CI_Model
{
	...
	public function get_join_num_row($tables, $jcond, $where)
	{
		$this->db->select('*');
		$this->db->where($where);
		for ($i = 0; $i < count($tables); $i++)
	...
	public function get_data_join_where($select, $tables, $jcond, $joinType, $where, $order = null)
	{
		$this->db->select($select);
		$this->db->where($where);
		for ($i = 0; $i < count($tables); $i++)
			$this->db->join($tables[$i], $jcond[$i], $joinType[$i]);
	...
	public function get_data_join_where_all($select, $tables, $jcond, $joinType, $where, $order = null)
	{
		$this->db->select($select);
		$this->db->where($where);
		for ($i = 0; $i < count($tables); $i++)
